...
- **Keep Microphone Ready**：默认关闭。开启后应用持续持有本地麦克风流，避免每次按住热键才启动设备；空闲 PCM 会立即丢弃，不缓存、不上传，只有按住热键期间的音频会发送给本次识别。
- **Input Method**：
...
诊断导出不会写出 Cookie、`device_id`、`web_id` 原文，可以安全地附在 issue 里帮助排障。麦克风音频只用于实时识别上传，不会落盘保存。开启 **Keep Microphone Ready** 时，空闲音频只在本地 callback 中被立即丢弃；不会被缓存或上传。
